...
int DBCFile::GetFieldType(int nField)
{
    if(nField < 0 || nField >= m_theType.size())
    {
        return -1;
    }

    return m_theType[nField];
}

string DBCFile::GetFieldName(int nField)
{
    if(nField < 0 || nField >= m_vecFieldName.size())
    {
        return string();
    }
    return m_vecFieldName[nField];
}
...
BOOL DBCFile::OpenFromMemoryImpl_Text(const CHAR* pMemory, const CHAR* pDeadEnd, const CHAR* szFileName)
{
    ...
    do
    {
        ...
        if(vRet[0] == "INT" || vRet[0] == "int" ||
           vRet[0] == "FLOAT" || vRet[0] == "float" ||
           vRet[0] == "STRING" || vRet[0] == "string")
        {
            break;
        }
        else
    ...
    //生成Field Types
    FILEm_pDataSaverTYPE vFieldsType;
    vFieldsType.resize(vRet.size());

    for(INT i = 0; i < (INT)vRet.size(); i++)
    {
        if(vRet[i] == "INT" || vRet[i] == "int")
            vFieldsType[i] = T_INT;
        else if(vRet[i] == "FLOAT" || vRet[i] == "float")
            vFieldsType[i] = T_FLOAT;
        else if(vRet[i] == "STRING" || vRet[i] == "string")
    ...
    _ConvertStringToVector(szLine, m_vecFieldName, "\t", TRUE, TRUE);
